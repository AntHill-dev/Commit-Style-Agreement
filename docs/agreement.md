...
[^4]: Под "должен" подразумевается - "опционально", но, конечно, если вы не собираетесь использовать МК.
