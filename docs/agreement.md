...
Где \<type_0> есть более ***общая*** категория, а \<type_1> ***конкретная*** область изменений. То есть есть вы сделали исправление опечатки (typo), то именно `typo` должна занять место \<type_1>. А то в какой из "подкатегории" вы это сделали - указывается в \<type_0>. Для примера выше должен[^4] указывается тип `[test:typo]`. 
