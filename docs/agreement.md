...
Первым делом коммит это заголовок. Именно заголовки читаются, как коммиты. Поэтому важно, что бы они были, как солдаты в строю, но за исключением того, что они должны идентифицировать изменения, хранящиеся внутри, к категориям указанных выше. Посему предлагается следующий синтаксис т.н. "шапки" коммита:

```
[<type>]<space><Заголовок>
```

Где \<type> собсна и связывает "контент" коммита и множество категорий (об это ниже). Также прошу обратить внимание, что тип указывается в некой "корзинке", а именно в квадратных скобках - `[ ]`. \<space> - понятно для чего; отделяет "корзинку" типа от заголовка. И главное - заголовок. Заголовок у нас на районе принято писать с большой буквы, потому что потому. Далее рассматривается правила письма типа и заголовка коммита.

### Разновидности типов для коммитов

Часто бывает так, что изменения внесённые в файл можно сразу определить в нескольким категориям, *~~или же кому то было лень сделать коммит раньше)~~*. В любом случае имеются ситуации, когда изменения можно отнести к разным категориям (но важно к каким именно, об этом ниже).

\- *Какие это такие ситуации?*

Например, когда вы изменили опечатку в README.md файле (typo, meta) или же изменили структуру файлов конфигурации проекта (style, project).

Для таких случаев у нас имеются т.н. "множества категорий", которые представляют из себя конкретизацию типов: если вы изменили документацию тестов, вы можете указать тип `style`, а можете конкретизировать область изменений добавив тип `test`. Но тут есть некоторые оговорки того, как это написать. Во-первых синтаксис использования "множества категорий" таков:

```
[<type_0>:<type_1>]
```

Где \<type_0> есть более ***общая*** категория, а \<type_1> ***конкретная*** область изменений. То есть есть вы сделали исправление опечатки (typo), то именно `typo` должна занять место \<type_1>. А то в какой из "подкатегории" вы это сделали - указывается в \<type_0>. Для примера выше должен* указывается тип `[test:typo]`. И тут **<u>важно не перепутать</u>**, потому что категории логически семантичны и мы не может указать тип, который бы говорил за себя, что он отвечает за "исправления документации в рефакторинге" -> `[docs:refact]`.

Также мы категорически предполагаем, что коммит не будет связан с более чем двумя категориями (*как это представить?*). Т.е. если указывается "множество категорий" то это множество имеет кардинальное число 2: тип 0 и тип 1 - вот и всё!

\#TODO: продолжить тему..
